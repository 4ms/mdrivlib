...
LEDDriverError PCA9685DmaDriver::start() {
	uint8_t driverAddr;
	LEDDriverError err;
...
void PCA9685DmaDriver::start_it_mode() {
	start();
...
// ...
LEDDriverError PCA9685DmaDriver::set_single_led(uint8_t led_element_number, uint16_t brightness) {
	uint8_t driver_addr;
	uint8_t data[5];
...
LEDDriverError PCA9685DmaDriver::set_rgb_led(uint8_t led_number, uint16_t c_red, uint16_t c_green, uint16_t c_blue) {
	uint8_t driverAddr;
	uint8_t data[13];
...
LEDDriverError PCA9685DmaDriver::reset_chip(uint8_t driverAddr) {
	LEDDriverError err;
	...
	if (err != LEDDriverError::None)
		return err;
	err = write_register(driverAddr, REG_MODE2,
						 0b00010001); // INVERT=1, OUTDRV=0, OUTNE=01
	if (err != LEDDriverError::None)
		return err;
...
LEDDriverError PCA9685DmaDriver::write_register(uint8_t driverAddr, uint8_t registerAddr, uint8_t registerValue) {
...
// is red + 1, blue = red + 2)
uint8_t PCA9685DmaDriver::get_red_led_element_id(uint8_t rgb_led_id) {
	return (rgb_led_id * 3) + (rgb_led_id / kNumRGBLEDsPerChip);
}

uint8_t PCA9685DmaDriver::get_chip_num(uint8_t rgb_led_id) {
