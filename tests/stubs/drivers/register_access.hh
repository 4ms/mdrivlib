...
struct RegisterBits : FakeMemory<address> {
	...
	static regsize_t read() {
		return AccessPolicyT::read(
			reinterpret_cast<volatile regsize_t *>(&FakeMemory<address>::FAKE_MEMORY), offset, mask);
	}
	static void write(regsize_t val) {
		AccessPolicyT::write(
			reinterpret_cast<volatile regsize_t *>(&FakeMemory<address>::FAKE_MEMORY), offset, mask, val);
	}
	static void set() {
		AccessPolicyT::set(reinterpret_cast<volatile regsize_t *>(&FakeMemory<address>::FAKE_MEMORY), mask);
	}
	static void clear() {
		AccessPolicyT::clear(reinterpret_cast<volatile regsize_t *>(&FakeMemory<address>::FAKE_MEMORY), mask);
	}
};

// RegisterSection<>: Bits in register specified by offset and width
// Deprecated, use RegisterBits with a mask directly
...
struct RegisterDualSetClear {
	static void set() {
		WriteOnly::set(reinterpret_cast<volatile regsize_t *>(&FakeMemory<SetAddress>::FAKE_MEMORY), SetMask);
	}
	static void clear() {
		WriteOnly::set(reinterpret_cast<volatile regsize_t *>(&FakeMemory<ClearAddress>::FAKE_MEMORY), ClearMask);
	}
};
