...
SaiPdmPeriph::Error SaiPdmPeriph::change_samplerate_blocksize(uint32_t sample_rate, uint32_t block_size) {
...
SaiPdmPeriph::Error SaiPdmPeriph::init() {
	_init_pins();
...
SaiPdmPeriph::Error SaiPdmPeriph::_config_rx_sai() {
	Error err = SAI_NO_ERR;
	...
	hsai_rx.Init.PdmInit.Activation = ENABLE;
...
void SaiPdmPeriph::_config_rx_dma() {
...
SaiPdmPeriph::Error SaiPdmPeriph::_init_sai_dma() {
...
void SaiPdmPeriph::set_rx_buffer(uint8_t *rx_buf_ptr, uint32_t frames_per_block) {
	rx_buf_ptr_ = rx_buf_ptr;
	rx_block_size_ = frames_per_block * saidef_.num_tdm_ins * 2;
}

void SaiPdmPeriph::set_rx_buffer_start(uint8_t *rx_buf_ptr, uint32_t block_size) {
	rx_buf_ptr_ = rx_buf_ptr;
	rx_block_size_ = block_size;
}

void SaiPdmPeriph::set_callbacks(Interrupt::ISRType &&tx_complete_cb, Interrupt::ISRType &&tx_half_complete_cb) {
	rx_tc_cb = std::move(tx_complete_cb);
	rx_ht_cb = std::move(tx_half_complete_cb);
}

void SaiPdmPeriph::start() {
	if (saidef_.mode == SaiConfig::RXMaster) {
...
void SaiPdmPeriph::_start_irq(IRQn_Type irqn) {
...
void SaiPdmPeriph::stop() {
	InterruptControl::disable_irq(rx_irqn);
	HAL_SAI_Abort(&hsai_rx);
}

void SaiPdmPeriph::pause() {
	InterruptControl::disable_irq(rx_irqn);
}

void SaiPdmPeriph::resume() {
	if (saidef_.mode == SaiConfig::RXMaster)
		InterruptControl::enable_irq(rx_irqn);
}

void SaiPdmPeriph::_sai_enable(SAI_HandleTypeDef *hsai) {
	hsai->Instance->CR1 = hsai->Instance->CR1 | (0x1UL << 16U);
}

void SaiPdmPeriph::_sai_disable(SAI_HandleTypeDef *hsai) {
	hsai->Instance->CR1 = hsai->Instance->CR1 & ~(0x1UL << 16U);
}
