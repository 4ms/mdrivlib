...
void SaiTdmPeriph::_config_rx_dma() {
	...
	hdma_rx.Instance = DMARegisters::get_channel(saidef_.dma_init_rx.DMAx, saidef_.dma_init_rx.channel);
...
void SaiTdmPeriph::_config_tx_dma() {
	...
	hdma_tx.Instance = DMARegisters::get_channel(saidef_.dma_init_tx.DMAx, saidef_.dma_init_tx.channel);
...
void SaiTdmPeriph::start() {
	if (saidef_.mode == SaiConfig::RXMaster) {
		...
		dma_isr_reg = &saidef_.dma_init_rx.DMAx->ISR;
		dma_ifcr_reg = &saidef_.dma_init_rx.DMAx->IFCR;
		_start_irq(rx_irqn);
	}
	if (saidef_.mode == SaiConfig::TXMaster) {
		...
		dma_isr_reg = &saidef_.dma_init_tx.DMAx->ISR;
		dma_ifcr_reg = &saidef_.dma_init_tx.DMAx->IFCR;
		_start_irq(tx_irqn);
	}
	if (saidef_.mode == SaiConfig::ExtSynced) {
...
void SaiTdmPeriph::_sai_enable(SAI_HandleTypeDef *hsai) {
	hsai->Instance->CR1 = hsai->Instance->CR1 | SAI_xCR1_SAIEN;
}
