...
struct UartTarget {
	...
	static bool set_baudrate(uint32_t baudrate_in_hz) {
		...
		if constexpr (Conf.base_addr == USART2_BASE)
			peripheral_clock_source = LL_RCC_USART2_CLKSOURCE;

		if constexpr (Conf.base_addr == USART3_BASE)
			peripheral_clock_source = LL_RCC_USART35_CLKSOURCE;

		if constexpr (Conf.base_addr == UART4_BASE)
			peripheral_clock_source = LL_RCC_UART4_CLKSOURCE;
		...
		if constexpr (Conf.base_addr == UART7_BASE)
			peripheral_clock_source = LL_RCC_UART78_CLKSOURCE;
