...
constexpr inline uint8_t PortA = 0;
constexpr inline uint8_t PortB = 1;
constexpr inline uint8_t PortC = 2;
constexpr inline uint8_t PortD = 3;
constexpr inline uint8_t PortE = 4;
constexpr inline uint8_t PortF = 5;
constexpr inline uint8_t PortG = 6;
constexpr inline uint8_t PortH = 7;
constexpr inline uint8_t PortJ = 9;
constexpr inline uint8_t PortK = 10;

// Rising/Falling edge trigger selection:
using RTSR = RegisterBits<ReadWrite, EXTI_BASE + offsetof(EXTI_TypeDef, RTSR), 0x00FFFFFF>;
using FTSR = RegisterBits<ReadWrite, EXTI_BASE + offsetof(EXTI_TypeDef, FTSR), 0x00FFFFFF>;

// Interrupt and Event enables:
using IMR = RegisterBits<ReadWrite, EXTI_BASE + offsetof(EXTI_TypeDef, IMR), 0x00FFFFFF>;
using EMR = RegisterBits<ReadWrite, EXTI_BASE + offsetof(EXTI_TypeDef, EMR), 0x00FFFFFF>;
using SWIER = RegisterBits<ReadWrite, EXTI_BASE + offsetof(EXTI_TypeDef, SWIER), 0x00FFFFFF>;

// Rising/Falling trigger pending (read, then write 1 to clear)
using PR = RegisterBits<ReadWrite, EXTI_BASE + offsetof(EXTI_TypeDef, PR), 0x00FFFFFF>;
